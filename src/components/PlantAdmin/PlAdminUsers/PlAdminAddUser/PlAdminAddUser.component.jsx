...
class PlantAdminCreateUserComponent extends Component {
  constructor(props){
    super(props); 
    this.state={
  ...
  getPlantDepartments=async()=>{
    const plantData=await PlantServices.getDepartmentsInPlant(this.props.plantId)
    plantData.departments.forEach(department => {
      department.isSelected=false
    });
    this.setState({plantDeparments:plantData.departments})
  }
  ...
  onSubmit=async()=>{
    ...
    const response=await UsersServices.createUser(userToCreate);
    ...
    if(this.props.accessLevel==3){
      ...
      return (
            <div >
              <Card>
                  <CardHeader>
                    <h5 style={{fontSize:'2vw'}}>Create User</h5>  
                  </CardHeader>
                  <CardBody>
...
export default PlantAdminCreateUserComponent;
